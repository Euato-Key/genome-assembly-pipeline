...
run_assembly() {
    ...
    # 使用 Flye 进行组装 (pacbio-hifi 模式)
    log "  开始 Flye 组装..."
    flye --pacbio-hifi "${HIFI_READS}" --genome-size 250m --out-dir "${PROJECT_NAME}_flye" --threads "${THREADS}" 2>> "${LOG_FILE}"
    
    log "  组装完成，Assembly: ${OUT_DIR}/${PROJECT_NAME}_flye/assembly.fasta"
    log "Step 4 完成。"
}
